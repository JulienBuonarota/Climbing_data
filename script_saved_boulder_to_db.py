...
# Insert each boulders into DB
gym_df = tgp.get_gym_table(DB_d)
gym = gym_df.loc[0]
            ...
            with conn.cursor() as cur:
                try:
                    ...
                    VALUES (%s, %s, %s,
                           ...
                           %s, %s, %s,
                           (SELECT id FROM gym_image WHERE gym_image.img_url = %s));""",
                                (data["grade"][0], data["grade"][1], int(gym["id"]),
                                 str(data["map_section"]),
        ...
        with conn:
            with conn.cursor() as cur:
                ...
                WHERE
                	...
                	AND sub_grade = %s
                	AND gym_id = %s
                	AND map_section_id = (SELECT id FROM gym_map_section WHERE gym_map_section.map_section = %s)
                	AND tag = %s
                	AND date_opened = %s
                	AND img_id = (SELECT id FROM gym_image WHERE gym_image.img_url = %s);""",
                            (data["grade"][0], data["grade"][1], int(gym["id"]),
                             str(data["map_section"]), "{" + ", ".join(data["tags"]) + "}",
                             data["date_opened"], data["img_url"]))
